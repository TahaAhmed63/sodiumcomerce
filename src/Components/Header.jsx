...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 bg-black text-dark">
      {/* Top Header */}
      <div className="flex items-center bg-white py-10 px-4 lg:px-8">
        {/* Left Section */}
      ...
      <div className="hidden lg:block bg-black border-t border-gray-800">
        <nav>
          <ul className="flex items-center justify-center space-x-6 px-4 py-2 text-sm text-white">
      ...
      {isMenuOpen && (
        <div className="lg:hidden absolute top-16 left-0 w-full bg-black border-t border-gray-800">
          <nav>
            <ul className="space-y-2 py-4 px-4 text-sm text-white">
              {categories.map((category) => (
                <li key={category}>
